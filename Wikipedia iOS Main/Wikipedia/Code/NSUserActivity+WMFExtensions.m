...
+ (nullable instancetype)wmf_activityForWikipediaScheme:(NSURL *)url {
    ...
    if ([url.host containsString:@"content"]) {
        return [self wmf_contentActivityWithURL:url];
    } else if ([url.host containsString:@"explore"]) {
        return [self wmf_exploreViewActivity];
    } else if ([url.host containsString:@"places"]) {
        
        // Check if url has location information
        if ([url.absoluteString containsString:@"$$$"]) {
            NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
            [standardUserDefaults setLocationReceived];
            [standardUserDefaults setCity: url.relativeString];
            [standardUserDefaults synchronize];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"DeepLinkNotification" object:self];
        }
        return [self wmf_placesActivityWithURL:url];
    } else if ([url.host containsString:@"saved"]) {
        return [self wmf_savedPagesViewActivity];
    } else if ([url.host containsString:@"history"]) {
