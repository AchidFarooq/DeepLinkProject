...
class PlacesViewController: ViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate, HintPresenting {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.deepLinkSend(notification:)), name: Notification.Name("DeepLinkNotification"), object: nil)
    ...
    @objc func deepLinkSend(notification: Notification) {
        checkIfDeepLinkShouldBeShown()
    }
    
    func checkIfDeepLinkShouldBeShown() {
        let userDefaults = UserDefaults.standard
        if userDefaults.getLocationReceived() {
            if let cityName = userDefaults.getCityName() {
    ...
    func handleCompletion(searchResults: [MWKSearchResult], siteURL: URL) -> [PlaceSearch] {
        ...
        if !completions.isEmpty {
            checkForDeeplink(completions[0])
        }

        return completions
    }
    
    func checkForDeeplink(_ placeSearch: PlaceSearch) {
        let userDefaults = UserDefaults.standard

        if userDefaults.getLocationReceived() {
            self.searching = false
            currentSearch = placeSearch
            performSearch(placeSearch)
            
            userDefaults.resetLocationReceived()
            userDefaults.resetCity()
        }
    }
